...
- **コンテキスト長**:`OLLAMA_CONTEXT_LENGTH=32768`(Qwen3 ネイティブ上限)で運用。既定 4096 では thinking モデルが途中打ち切りになるため引き上げ済み。`OLLAMA_KV_CACHE_TYPE=q8_0` + `OLLAMA_FLASH_ATTENTION=1` で KV キャッシュを量子化し、32k 文脈(q8_0 で ~2.6GB)を 14B+埋め込みと同居させて 16GB に収める。
